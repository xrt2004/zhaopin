...
class MsgModel extends Model{
	...
	public function msg_send($data,$user){
		...
		if($user['utype'] == 1){
			...
			$data['to_name'] = M('MembersInfo')->where(array('uid'=>$data['touid']))->getfield('realname');
			if($data['pid']){
    	...
    	//写入会员日志
		write_members_log($user,1026,$data['pid'],$data['touid'],$data['message']);
	...
	public function msg_list($user,$s = true,$pagesize=20){
		...
		if($msgList){
			if($user['utype'] == 1){
				...
				$members_info = M('MembersInfo')->where(array('uid'=>array('in',$mids)))->getfield('uid,realname');
				$company = M('CompanyProfile')->where(array('uid'=>array('in',$cids)))->getfield('uid,companyname,logo');
				$resume = M('Resume')->where(array('uid'=>array('in',$mids)))->order('def desc')->group('uid')->getfield('uid,id');
				foreach ($msgList as $key=>$val) {
					if($val['fromuid'] == $user['uid']){
						...
						$msgList[$key]['to_name'] = $members_info[$val['touid']]?:$members[$val['touid']]['username'];
						$msgList[$key]['to_url'] = $resume[$val['touid']] ? url_rewrite('QS_resumeshow',array('id'=>$resume[$val['touid']])) : '';
					}else{
						...
						$msgList[$key]['from_name'] = $members_info[$val['fromuid']]?:$members[$val['fromuid']]['username'];
						$msgList[$key]['from_url'] = $resume[$val['fromuid']] ? url_rewrite('QS_resumeshow',array('id'=>$resume[$val['fromuid']])) : '';
					}
				}
			}else{
				foreach ($msgList as $key=>$val) {
	...
	public function smsg_list($sids,$user,$s = true){
		...
		if($user['utype'] == 1){
			...
			foreach ($msgsList as $val) {
				if($val['fromuid'] == $user['uid']){
					$val['to_name'] = $members_info[$val['touid']]?:$members[$val['touid']]['username'];
					...
					$val['from_name'] = $members_info[$val['fromuid']]?:$members[$val['fromuid']]['username'];
	...
	public function msg_del($id,$user){
    	...
    	//写入会员日志
		write_members_log($user,1027,$id);
